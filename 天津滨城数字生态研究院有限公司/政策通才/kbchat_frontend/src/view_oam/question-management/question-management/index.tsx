...
export default function QuestionManagement() {
	...
	const columns = [
		...
		},
		{
			...
			sorter: true,
			render: (_text, _record, _index) => (
				<Tooltip title={((_text) && (_text)?.length>20)?(_text):null} overlayClassName='my-model-table'>
	...
	const handleView = (index) => {
		...
		history.push({
			pathname: '/oam/question-management/question-management/detail/' + data[index].id,
		})
	};
	// 展示删除弹框
	...
	return (
		...
		<div className={"w-full bg-white flex flex-col"}>
			...
			<div className={"w-full pl-8 pr-8 pt-6 flex flex-col rounded-lg"}>
				<span className={"text-base font-medium"}>问题管理</span>
				{/*搜索和按钮*/}
				<div className={"mt-4 w-full h-[36px] flex justify-between items-center"}>
					{/*search框筛选*/}
					<SearchBig placeholder='请输入问题/提问人'
					           ml={false}
					           style={{width: '284px'}}
