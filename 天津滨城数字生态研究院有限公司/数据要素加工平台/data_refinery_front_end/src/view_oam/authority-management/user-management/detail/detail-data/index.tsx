...
export default function DetailData() {
	...
	const handleView = (index) => {
		...
		if(selectedItem.current.resource_type === '官方数据') {
			window.open(location.origin + '/oam/data-management/official-data/detail/' + selectedItem.current.id)
		} else {
			window.open(location.origin + '/oam/data-management/user-data/detail/' + selectedItem.current.id)
		}
	};
	// @ts-ignore
		...
		return new Promise<any>((resolve, _reject) => {
			setTimeout(() => {
				...
				for(let i = 0; i < data.list.length; i++) {
					...
					data.list[i].type_desc = dataTypeList[index%dataTypeList.length].text
					...
					data.list[i].industry = applicationFieldList[index%applicationFieldList.length]
				}
				const res = {
					"code": 0,
					"msg": "success",
	...
	return (
		...
		<div className={"w-full flex flex-col pl-8 pr-8"}>
			...
			<DraggableModalPrompt
				...
				<span className="text-sm text-black">确定批量取消授权<span className="text-main pl-1 pr-1">{selectedRowKeys?.length}</span>个数据吗？</span>
			</DraggableModalPrompt>
			{/*取消授权弹框*/}
			<DraggableModalPrompt
				...
				<span className="text-sm text-black">确定取消授权<span className="text-main pl-1 pr-1">{selectedItem.current?.name}</span>数据吗？</span>
			</DraggableModalPrompt>
		</div>
	)
}
