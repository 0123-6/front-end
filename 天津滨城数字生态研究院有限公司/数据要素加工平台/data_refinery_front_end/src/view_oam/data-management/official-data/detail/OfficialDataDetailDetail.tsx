...
export default function OfficialDataDetailDetail(props:IProps) {
	// state
				...
				<div className={"w-full flex"}>
					...
					<div className={"flex-1 ml-7 flex flex-col"}>
						...
						<div className={"mt-[5px] w-full flex"}>
							<Tooltip title={(data?.description && data?.description.length>90)?data?.description:null}
							         overlayClassName='my-model-detail-desc'>
							...
							</Tooltip>
							{
								data?.status_desc &&
								<div className={"flex items-start"}>
									<ButtonMain text={'上线'} click={handleOnline} style={{width: '86px',height:'32px'}} disabled={!(data.status_desc==='未上线'||data.status_desc==='已下线')} className={"ml-2"}/>
									<ButtonFifth text={'编辑'} click={()=>changeMode('编辑')} style={{width: '86px',height:'32px'}} disabled={!(data.status_desc==='未上线' || data.status_desc==='已下线')} className={"ml-[11px]"}/>
								</div>
							}
						</div>
						...
						<div className={"mt-[18px] flex items-center"}>
							{/*提供者*/}
							<div className={"mr-[38px] flex items-center"}>
