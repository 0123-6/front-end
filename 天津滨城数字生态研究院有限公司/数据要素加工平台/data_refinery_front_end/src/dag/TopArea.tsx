...
import {MODELS, NsGraphCmd, useXFlowApp, XFlowGraphCommands} from "@antv/xflow";
import {useHistory} from 'react-router-dom'
import Write from "../icon/write";
import EditModal from "../view/experiment/components/EditModal";
import {getExperimentById, saveExperiment} from "../axios/xflow";
import CloseExperiment from "./components/CloseExperiment";
...
export const TopArea = (props:IProps) => {
  ...
  const experimentInfo = useRef(null)
  ...
  const editModalOnOk = (data) => {
    ...
    saveExperiment(params).then(res => {
      ...
      // app.executeCommand(XFlowGraphCommands.LOAD_META.id,{
      //   meta:
      // })
    }).catch(err => {
      console.log(err)
  ...
  useEffect(()=>{
    ...
    MODELS.GRAPH_META.useValue(app.modelService).then(res=>{
      ...
      if (_experimentInfo?.name) {
        setExperimentName(_experimentInfo.name)
        setExperimentDescription(_experimentInfo.description)
        const _testObject = {
          cpu: _experimentInfo?.comp_info?.cpu,
          memory: _experimentInfo?.comp_info?.memory,
        }
        setTestObject(_testObject)
        experimentInfo.current = _experimentInfo
      } else {
        getExperimentById(_experimentInfo).then(res => {
          setExperimentName(res.data.name)
          setExperimentDescription(res.data.description)
          const _testObject = {
            cpu: res.data?.comp_info?.cpu,
            memory: res.data?.comp_info?.memory,
          }
          setTestObject(_testObject)
  ...
  const clickUndo = () => {
    // app.executeCommand<NsGraphCmd.UndoCmd.IArgs>(
  ...
  const clickFile = () => {
    document.getElementById('btn-save-processing-flow')!.click()
  }
  ...
  return (
    ...
    <div className="w-full h-full flex justify-between items-center bg-white border-b border-white-divide">
      ...
      <div className="h-full flex items-center">
        {/*logo*/}
        <img className="hover:cursor-pointer" onClick={goHome} src={logoPng} style={{marginLeft: '17px'}} width="150px"
             height="37px" alt=''/>
        {/*名称*/}
        <div style={{marginLeft: '75px'}}
             className="flex items-center">
          {/*文字*/}
          <span className="text-base font-semibold text-hidden" style={{color: '#323232', maxWidth:'318px'}}>
            {experimentName}
          </span>
          {
            !isTest &&
            <div style={{marginLeft: '10px'}}
                 className='p-1 flex items-center write-icon-default hover:text-main hover:cursor-pointer active:text-main-focus'
                 onClick={_e=>setShowEditModal(true)}>
              <Write/>
            </div>
          }
          {
            isTest && testObject?.cpu && testObject?.memory &&
            <div className={"text-sm"}
                 style={{marginLeft:'57px'}}>
              <span className={"flex items-center"}>测试资源：{testObject?.cpu} vCPU {testObject?.memory} GiB</span>
            </div>
          }
        </div>
      </div>
      ...
      <div className="h-full flex items-center">
        ...
        <div className="h-full flex items-center">
          ...
          <Tooltip title="上一步" mouseEnterDelay={0} mouseLeaveDelay={0}>
            <div style={{width: '32px', height: '32px'}}
                 className="rounded hover:bg-white-bg hover:cursor-pointer flex justify-center items-center text-close hover:text-main-hover active:text-main-focus"
                 onClick={clickUndo}>
                ...
                <g id="页面-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                  <g id="按钮备份" transform="translate(-899.000000, -515.000000)">
                    <g id="arrow-go-back-line-(1)" transform="translate(899.000000, 515.000000)">
                      ...
                      <path
                        d="M5.828,7 L8.364,9.536 L6.95,10.95 L2,6 L6.95,1.05 L8.364,2.464 L5.828,5 L13,5 C17.418278,5 21,8.581722 21,13 C21,17.418278 17.418278,21 13,21 L4,21 L4,19 L13,19 C16.3137085,19 19,16.3137085 19,13 C19,9.6862915 16.3137085,7 13,7 L5.828,7 Z"
          ...
          <Tooltip title="下一步" mouseEnterDelay={0} mouseLeaveDelay={0}>
            <div style={{width: '32px', height: '32px', marginLeft: '10px'}}
                 className="rounded hover:bg-white-bg hover:cursor-pointer flex justify-center items-center text-close hover:text-main-hover active:text-main-focus"
                 onClick={clickRedo}>
              <svg width="22px" height="22px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg"
                   xmlnsXlink="http://www.w3.org/1999/xlink">
                <g id="页面-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                  <g id="按钮备份" transform="translate(-955.000000, -515.000000)">
                    <g id="arrow-go-forward-line" transform="translate(955.000000, 515.000000)">
                      <polygon id="路径" points="0 0 24 0 24 24 0 24"></polygon>
                      <path
                        d="M18.172,7 L11,7 C7.6862915,7 5,9.6862915 5,13 C5,16.3137085 7.6862915,19 11,19 L20,19 L20,21 L11,21 C6.581722,21 3,17.418278 3,13 C3,8.581722 6.581722,5 11,5 L18.172,5 L15.636,2.464 L17.05,1.05 L22,6 L17.05,10.95 L15.636,9.536 L18.172,7 Z"
                        id="路径" fill="currentColor" fill-rule="nonzero"></path>
                    </g>
                  </g>
                </g>
          ...
          {/*分割线*/}
          <div style={{marginLeft: '10px', width: '1px', height: '18px'}}
          ...
          <Tooltip title="保存实验" mouseEnterDelay={0} mouseLeaveDelay={0}>
            <div style={{width: '32px', height: '32px', marginLeft: '10px'}}
                 className="rounded hover:bg-white-bg hover:cursor-pointer flex justify-center items-center text-close hover:text-orange-hover active:text-orange-focus"
                 onClick={clickSave}>
              <svg width="22px" height="22px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg"
                   xmlnsXlink="http://www.w3.org/1999/xlink">
                <g id="页面-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
          ...
          {/*文件*/}
          {
            ...
            <Tooltip title="保存加工流程" mouseEnterDelay={0} mouseLeaveDelay={0}>
              <div style={{width: '32px', height: '32px', marginLeft: '10px'}}
                   className="rounded hover:bg-white-bg hover:cursor-pointer flex justify-center items-center text-close hover:text-blue-icon-hover active:text-blue-icon-focus"
                   onClick={clickFile}>
                <svg width="22px" height="22px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg"
                     xmlnsXlink="http://www.w3.org/1999/xlink">
                  <g id="页面-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g id="按钮备份" transform="translate(-1156.000000, -515.000000)">
                      <g id="file-text-line-(1)" transform="translate(1156.000000, 515.000000)">
                        <polygon id="路径" points="0 0 24 0 24 24 0 24"></polygon>
                        <path
                          d="M21.0000247,8 L21.0000247,20.993 C21.001863,21.2582184 20.8982888,21.5133136 20.7120638,21.7021642 C20.5258387,21.8910147 20.2722185,21.99815 20.007,22 L3.993,22 C3.44497153,22 3.00055189,21.5560282 3,21.008 L3,2.992 C3,2.455 3.449,2 4.002,2 L14.997,2 L21.0000247,8 Z M19,9 L14,9 L14,4 L5,4 L5,20 L19,20 L19,9 Z M8,7 L11,7 L11,9 L8,9 L8,7 Z M8,11 L16,11 L16,13 L8,13 L8,11 Z M8,15 L16,15 L16,17 L8,17 L8,15 Z"
                          id="形状" fill="currentColor" fill-rule="nonzero"></path>
                      </g>
                    </g>
                  </g>
          ...
          <Tooltip title="退出实验" mouseEnterDelay={0} mouseLeaveDelay={0}>
            <div style={{width: '32px', height: '32px', marginLeft: '10px'}}
                 className="rounded hover:bg-white-bg text-close hover:text-red hover:cursor-pointer active:text-red-hover flex justify-center items-center"
                 onClick={clickClose}>
              <svg width="28px" height="28px" viewBox="0 0 34 34" version="1.1" xmlns="http://www.w3.org/2000/svg"
                ...
                <g id="页面-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                  <g id="按钮备份" transform="translate(-1208.000000, -512.000000)">
                    <g id="edit-circle-line"
                       transform="translate(1224.970563, 528.970563) rotate(-45.000000) translate(-1224.970563, -528.970563) translate(1212.970563, 516.970563)">
                      <polygon id="路径" points="0 0 24 0 24 24 0 24"></polygon>
                      <path
        ...
        {/*头像区域*/}
        <div className="mr-11 h-full flex items-center" style={{marginLeft: '158px'}}>
          <UserComponent newTab={true}/>
        </div>
      </div>
      {/*编辑弹框*/}
      <EditModal data={{name:experimentName,description: experimentDescription}}
                 show={showEditModal}
                 onOk={(data)=>editModalOnOk(data)}
                 onCancel={editModalOnCancel}/>
      <CloseExperiment show={showCloseModal}
                       onOk={closeModalOnOk}
