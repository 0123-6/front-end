 ...
 * @LastEditTime: 2023-07-07 17:11:21
...
export default {
  ...
  methods: {
    handleChange() {
      const metaCheckedNodeList = this.$refs[this.refStr].getCheckedNodes();
      const checkedNodeList = metaCheckedNodeList.filter((item) => !(item.parent && item.parent.checked)); // 核心
      const comparisonCheckedNodeList = metaCheckedNodeList.filter((item) => item.checked); // 处理同一个父子级对比
      ...
      this.comparisonPathList = comparisonCheckedNodeList.map((item) => item.path);
      this.comparisonNameList = comparisonCheckedNodeList.map((item) => item.label);
      this.comparisonNameStr = comparisonCheckedNodeList.map((item) => item.label).join(',');
      // 如果只查全选的父级，可以通过这两个方法
      this.$emit('getCurrentLabel', this.nameList);
      this.$emit('getCurrentKey', this.pathList);
      // 同一级父子级对比
      this.$emit('getComparisonKey', this.comparisonPathList);
      this.$emit('getComparisonLabel', this.comparisonNameList);
