...
export default {
  ...
  watch: {
    chartData: {
      ...
      handler(val) {
        ...
        const collapseLevel = this.getCollapseLevel(data[0]);
        this.handleData(data, collapseLevel);
        if (this.graph) {
          this.graph.data(data[0]);
          this.graph.render();
          let paddingY = this.getPaddingY(data[0], collapseLevel);
  ...
  methods: {
    getCollapseLevel(data) {
      let num = 0;
      data.children.forEach((c) => {
        num += c.children.length;
      });
      return num > 20 ? 2 : 3;
    },
    getPaddingY(data, level) {
      let paddingY = 0;
    ...
    handleData(data, level) {
      data?.forEach((item) => {
        if (item.children && item.children.length > 0) {
          item.collapsed = item.chainLevel >= level;
          this.handleData(item.children, level);
        }
      });
    },
    // 全屏事件处理函数
