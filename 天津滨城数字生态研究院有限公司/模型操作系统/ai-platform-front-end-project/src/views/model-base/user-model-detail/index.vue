      ...
      >
        ...
        <div class="flex align-center border-bottom padding-bottom-24 margin-top-24" style="width: 100%;">
          ...
          <div v-if="!preview" class="flex flex-1 justify-center align-center">
            ...
            <el-button v-if="permissionParams.status === 'ON_LINE'" :disabled="!modelInfo.hasUnionDataSet" class="operation-primary-button font-size-14" style="height: 40px;" @click="clickUseDemo">体验Demo</el-button>
          </div>
        </div>
        <!--模型描述-->
        ...
        <!--api服务接口-->
        <div v-if="!preview && permissionParams.status === 'ON_LINE'" class="flex flex-direction border-bottom margin-top-24">
          <title-style title="API服务接口" :font-size="16" :end-icon="true" :source-route="'user-model'" :model-id="$route.params.id" />
          <div class="flex align-center margin-bottom-16" style="width: 100%;height: 40px;">
            ...
            >
              ...
              <el-tooltip class="item" effect="dark" content="复制" placement="top">
                <span v-show="showCopy && (modelInfo.api && !(modelInfo.api.url === '未授权' || modelInfo.api.url === '已过期' || modelInfo.api.url === '已下线'))" class="hand margin-right-16 copyItem" data-clipboard-action="copy" :data-clipboard-text="modelInfo.api.url" @click="copySuccess">
...
export default {
  ...
  mounted() {
    ...
    console.log('this.permissionParams', this.permissionParams);
  },
  methods: {
    copySuccess() {
      // 复制文本内容
    ...
    returnModelBase() {
      // this.$router.back();
      this.$router.push('/permission-management/user-management/list');
    },
    initECharts() {
