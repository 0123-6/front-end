 ...
 * @LastEditTime: 2022-09-27 13:58:42
...
<template>
  <div class="common-dataSet-label-list-container">
    <DetailCard :card-detail="dataSetDetail" :is-show-fork="false" :info-type="'data-set'" :tag-type="tagType" @back="backList" />
...
import { getDataSetPlatDetail, getDataSetPlatTaskList, getDataSetEditDetail } from '@/api/data-set';
...
export default {
  ...
  methods: {
    // 返回
    backList() {
      this.$router.push({ path: `/data-manager/data-set/common-detail/${this.webType}/${this.$route.params.id}/${this.$route.params.tagType}` });
    },
    // 获取详情信息
    getCardDetail() {
    ...
    toPrev() {
      ...
      if (index === -1 || index === 0) {
        getDataSetPlatTaskList({
          ...
          if (res.code === '000000') {
            this.$router.push({ path: `/data-manager/data-set/common-label/${this.webType}/${this.$route.params.id}/${this.$route.params.tagType}/detail/${res.data.tasks[49].id}/${this.currentIndex - 1}` });
        ...
        this.$router.push({ path: `/data-manager/data-set/common-label/${this.webType}/${this.$route.params.id}/${this.$route.params.tagType}/detail/${this.IDList[index - 1]}/${this.currentIndex - 1}` });
        this.currentIndex += -1;
      }
    },
    toNext() {
      ...
      if (index === -1 || index === 49) {
        if (this.pageNumber === Math.ceil(this.dataSetDetail.taskNumber / 50)) {
          this.$router.push({ path: `/data-manager/data-set/common-label/${this.webType}/${this.$route.params.id}/${this.$route.params.tagType}/detail/${this.IDList[0]}/${this.currentIndex + 1}` });
          this.currentIndex += 1;
        } else {
          getDataSetPlatTaskList({
            labelStudioProjectId: this.dataSetDetail.labelStudioProjectId,
            ...
            if (res.code === '000000') {
              this.$router.push({ path: `/data-manager/data-set/common-label/${this.webType}/${this.$route.params.id}/${this.$route.params.tagType}/detail/${res.data.tasks[0].id}/${this.currentIndex + 1}` });
              this.$store.commit('dataSet/CLEAR_ID_LIST');
              this.$store.commit('dataSet/ADD_ID_LIST', { tableData: res.data.tasks, pageNum: this.pageNumber + 1 });
              this.currentIndex += 1;
            }
