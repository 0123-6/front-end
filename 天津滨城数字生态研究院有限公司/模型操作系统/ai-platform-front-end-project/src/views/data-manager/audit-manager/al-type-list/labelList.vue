 ...
 * @LastEditTime: 2022-08-31 09:41:58
...
<template>
  <div class="common-dataSet-label-list-container">
    <DetailCard :card-detail="dataSetDetail" :is-show-fork="false" :info-type="'audit'" @back="backList" />
...
import { dataCheckInfo } from '@/api/data-check';
...
export default {
  ...
  methods: {
    // 返回
    backList() {
      this.$router.push({ path: `/data-manager/audit/al-type-list/${this.$route.params.id}/audit` });
    },
    // 获取详情信息
    getCardDetail() {
    ...
    toPrev() {
      ...
      if (index === -1 || index === 0) {
        getDataSetPlatTaskList({
          ...
          if (res.code === '000000') {
            this.$router.push({ path: `/data-manager/audit/common-label/${this.$route.params.id}/detail/${res.data.tasks[49].id}/${this.currentIndex - 1}` });
        ...
        this.$router.push({ path: `/data-manager/audit/common-label/${this.$route.params.id}/detail/${this.IDList[index - 1]}/${this.currentIndex - 1}` });
        this.currentIndex += -1;
      }
    },
    toNext() {
      ...
      if (index === -1 || index === 49) {
        if (this.pageNumber === Math.ceil(this.dataSetDetail.taskNumber / 50)) {
          this.$router.push({ path: `/data-manager/audit/common-label/${this.$route.params.id}/detail${this.IDList[0]}/${this.currentIndex + 1}` });
          this.currentIndex += 1;
        } else {
          getDataSetPlatTaskList({
            labelStudioProjectId: this.dataSetDetail.labelStudioProjectId,
            ...
            if (res.code === '000000') {
              this.$router.push({ path: `/data-manager/audit/common-label/${this.$route.params.id}/detail/${res.data.tasks[0].id}/${this.currentIndex + 1}` });
              this.$store.commit('dataSet/CLEAR_ID_LIST');
              this.$store.commit('dataSet/ADD_ID_LIST', { tableData: res.data.tasks, pageNum: this.pageNumber + 1 });
              this.currentIndex += 1;
            }
