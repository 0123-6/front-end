...
export default {
  ...
  computed: {
    ...mapGetters(['IDList', 'pageNumber', 'labelStudioUrl'])
  },
  mounted() {
    this.getCardDetail();
  ...
  methods: {
    ...
    handleView(data, index) {
      ...
      this.$router.push({ path: `/data-manager/push/common-label/${this.$route.params.id}/detail/${data.id}/${rowIndex}` });
    },
    // 分页
    ...
    returnBack() {
      this.$router.push({ path: `/data-manager/push/list` });
    },
    // 文本悬浮显示
  ...
  .crumbs{
    ...
    >div:nth-child(1){
      ...
      >span:nth-child(2){
        ...
        border-radius: 47px 1px 35px 1px;
      }
    }
  }
