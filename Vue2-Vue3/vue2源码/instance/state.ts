...
import { shallowReactive } from 'v3'
...
export function initState(vm: Component) {
  ...
  // 如果存在watch,初始化watch
  if (vm.$options.watch && vm.$options.watch !== nativeWatch) {
...
function initData(vm: Component) {
  // 获取data
  vm._data = isFunction(vm.$options.data) ? getData(vm.$options.data, vm) : vm.$options.data || {}
  // 遍历key，在vm上设置映射
...
function initComputed(vm: Component, computed: Object) {
  ...
  // 如果vm._computedWatchers不存在，那么创建
  ...
  for (const key in computed) {
    ...
    vm._computedWatchers[key] = new Watcher(vm, getter, noop, { lazy: true })
...
export function defineComputed(
  ...
  Object.defineProperty(target, key, {
    ...
    set: isFunction(userDef) ? noop : (userDef.set || noop),
  })
}
