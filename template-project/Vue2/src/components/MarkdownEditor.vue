...
import {ISelectFileProps, ableSelectFileByClick, exportFile } from '@/util/file.ts';
...
export default defineComponent({
	...
	mounted() {
		...
		if (this.showImportAndExport) {
			const importProps: ISelectFileProps = {
				element: this.$refs.importMarkdownFileButtonRef!,
				accept: '.md,.markdown',
				callback: async file => {
					const text = await file.text()
					this.editor!.setMarkdown(text, false)
				},
				callbackError: text => {
					this.$message({
						type: 'error',
	...
	methods: {
		exportMarkdownFile() {
			const text = (this.editor as Editor).getMarkdown()
